...
module.exports = async function layoutSmoke(page, artifacts) {
  ...
  for (const width of [1008, 1440, 1920]) {
    ...
    const grid=await page.locator('.home-continue-grid').evaluate(el=>{const parent=el.getBoundingClientRect(),boxes=[...el.children].map(c=>c.getBoundingClientRect()),firstRow=boxes.filter(r=>Math.abs(r.top-boxes[0].top)<2);return{right:parent.right,rowRight:firstRow.at(-1).right,widths:boxes.map(r=>r.width),heights:boxes.map(r=>r.height)}})
    assert.ok(Math.abs(grid.right-grid.rowRight)<2,'A complete Continue listening row must reach the content edge')
