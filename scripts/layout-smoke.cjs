...
module.exports = async function layoutSmoke(page, artifacts) {
  ...
  for (const width of [1008, 1440, 1920]) {
    ...
    assert.equal(await page.locator('.continue-shelf').count(),0,'All books has no duplicate continuation section')
    ...
    assert.equal(await page.locator('.continue-grid').evaluate(el=>getComputedStyle(el).display),'grid','Library continuation is a grid')
    assert.equal(await page.locator('.continue-shelf .home-shelf-track').count(),0,'No carousel on the Continue listening tab')
    ...
    assert.ok(await page.locator('.continue-grid .resume-card').evaluateAll(cards=>cards.every(c=>{const card=c.getBoundingClientRect(),meter=c.querySelector('.daily-meter').getBoundingClientRect();return meter.right<=card.right&&meter.bottom<=card.bottom})), 'Progress stays inside every grid card')
