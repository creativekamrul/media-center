...
async function main() {
  ...
  const mutations = []
  let failLibraries = false, failReadRequests=false
  const server = createServer(async (req, res) => {
    requests.push(req.url)
    const url = new URL(req.url, 'http://localhost')
    res.setHeader('Content-Type', 'application/json')
    if(failReadRequests){res.statusCode=503;return res.end('{}')}
    ...
    if (url.pathname.startsWith('/rest/')) {
      ...
      if(url.pathname.includes('getAlbumList2')&&form.get('type')==='recent')data.albumList2={album:Array.from({length:65},(_,i)=>({id:'recent-'+i,name:'Recent record '+(i+1),artist:'Fixture Artist',songCount:1,year:2026})).slice(Number(form.get('offset')||0),Number(form.get('offset')||0)+Number(form.get('size')||60))}
      if(url.pathname.includes('search3')&&form.get('artistCount')!=='0') data.searchResult3.artist=[{id:'artist',name:'Test Artist',albumCount:1}].filter(a=>a.name.toLowerCase().includes((form.get('query')||'').toLowerCase()))
  ...
  try {
    ...
    if(process.env.MEDIA_CENTER_UI150_ONLY||process.env.MEDIA_CENTER_PLAYER_POLISH_ONLY||process.env.MEDIA_CENTER_RELEASE_UI_ONLY||process.env.MEDIA_CENTER_UI140_ONLY||process.env.MEDIA_CENTER_VIEW_PINS_ONLY||process.env.MEDIA_CENTER_LIBRARIES_ONLY||process.env.MEDIA_CENTER_CONTROLS_ONLY||process.env.MEDIA_CENTER_REFINEMENTS_ONLY||process.env.MEDIA_CENTER_APP_STYLES_ONLY||process.env.MEDIA_CENTER_STYLES_ONLY||process.env.MEDIA_CENTER_UI_CLEANUP_ONLY||process.env.MEDIA_CENTER_CONSISTENCY_ONLY||process.env.MEDIA_CENTER_STUDIO_ONLY||process.env.MEDIA_CENTER_PERSONAL_ONLY||process.env.MEDIA_CENTER_MIXES_ONLY||process.env.MEDIA_CENTER_BUGS_ONLY||process.env.MEDIA_CENTER_POLISH_ONLY){
      ...
      if(process.env.MEDIA_CENTER_VIEW_PINS_ONLY){await page.reload();await require('./view-pins-smoke.cjs')({page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_UI150_ONLY){await page.reload();await require('./ui-150-smoke.cjs')({desktop,page,artifacts,setOffline:value=>{failReadRequests=value}});await require('./installer-quit-smoke.cjs')({desktop,page,profileArg});assert.deepEqual(errors,[]);writeFileSync(resolve(artifacts,'ui-150-smoke.json'),JSON.stringify({passed:true,version:require('../package.json').version,packagedExecutable:process.env.MEDIA_CENTER_EXECUTABLE,rendererErrors:errors},null,2));return}
      if(process.env.MEDIA_CENTER_PLAYER_POLISH_ONLY){await page.reload();await require('./player-polish-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_RELEASE_UI_ONLY){
        await page.reload()
        await require('./app-styles-smoke.cjs')({desktop,page,artifacts})
        await require('./listening-refinements-smoke.cjs')({desktop,page,artifacts})
        await require('./view-pins-smoke.cjs')({page,artifacts})
        await require('./music-libraries-smoke.cjs')({page,artifacts,serveWav})
        await require('./ui-140-smoke.cjs')({desktop,page,artifacts})
        await require('./player-polish-smoke.cjs')({desktop,page,artifacts})
        await require('./ui-150-smoke.cjs')({desktop,page,artifacts,setOffline:value=>{failReadRequests=value}})
        await require('./installer-quit-smoke.cjs')({desktop,page,profileArg})
        assert.deepEqual(errors,[])
        writeFileSync(resolve(artifacts,'release-ui-smoke.json'),JSON.stringify({passed:true,version:require('../package.json').version,packagedExecutable:process.env.MEDIA_CENTER_EXECUTABLE,rendererErrors:errors},null,2));return
      }
      if(process.env.MEDIA_CENTER_UI140_ONLY){await page.reload();await require('./ui-140-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_LIBRARIES_ONLY){await page.reload();await require('./music-libraries-smoke.cjs')({page,artifacts,serveWav});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_CONTROLS_ONLY){await page.reload();await require('./immersive-controls-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_REFINEMENTS_ONLY){await page.reload();await require('./listening-refinements-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_APP_STYLES_ONLY){await page.reload();await require('./app-styles-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_STYLES_ONLY){await page.reload();await require('./consistency-smoke.cjs')({desktop,page,artifacts});await require('./immersive-styles-smoke.cjs')({desktop,page,artifacts});await require('./installer-quit-smoke.cjs')({desktop,page,profileArg});assert.deepEqual(errors,[]);return}
      if(process.env.MEDIA_CENTER_UI_CLEANUP_ONLY){await page.reload();await require('./ui-cleanup-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);return}
      ...
      if(process.env.MEDIA_CENTER_PERSONAL_ONLY){await page.reload();await require('./personal-library-smoke.cjs')({desktop,page,artifacts});assert.deepEqual(errors,[]);writeFileSync(resolve(artifacts,'personal-library-smoke.json'),JSON.stringify({passed:true,nativeMpvTested:!!mpvPath,rendererErrors:errors},null,2));return}
    ...
    await require('./navigation-smoke.cjs')({desktop,page,artifacts})
    ...
    await page.getByLabel('Music library',{exact:true}).waitFor()
    failLibraries = true
    await page.getByRole('button', {name:'Refresh libraries',exact:true}).click()
    await page.getByRole('button', {name:'Retry connections',exact:true}).waitFor()
    assert.equal(await page.getByLabel('Music library',{exact:true}).isVisible(),true)
    ...
    assert.equal(await page.locator('.detail-hero .art-shape').count(),1,'Missing audiobook artwork keeps the generated in-app cover')
    assert.equal(await page.locator('.detail-hero .art img').count(),0,'Discord fallback does not replace the generated audiobook cover')
    await page.getByRole('button', { name: 'Podcasts', exact: true }).click()
    ...
    assert.equal(await page.locator('.detail-hero .art-shape').count(),1,'Missing podcast artwork keeps the generated in-app cover')
    assert.equal(await page.locator('.detail-hero .art img').count(),0,'Discord fallback does not replace the generated podcast cover')
    ...
    if (mpvPath) {
      ...
      await page.locator('.expanded-art .more-options-trigger').click();await page.locator('.expanded-art').getByRole('button', { name: 'Listen later', exact: true }).click()
      ...
      await require('./view-navigation.cjs')(page,'Folders')
    ...
    if (mpvPath) {
      ...
      await require('./personal-library-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./mixes-smoke.cjs')({desktop,page:restoredPage,artifacts})
      ...
      await require('./ui-cleanup-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./immersive-styles-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./immersive-controls-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./app-styles-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./listening-refinements-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./view-pins-smoke.cjs')({page:restoredPage,artifacts})
      await require('./music-libraries-smoke.cjs')({page:restoredPage,artifacts,serveWav})
      await require('./ui-140-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./player-polish-smoke.cjs')({desktop,page:restoredPage,artifacts})
      await require('./ui-150-smoke.cjs')({desktop,page:restoredPage,artifacts,setOffline:value=>{failReadRequests=value}})
      await require('./installer-quit-smoke.cjs')({desktop,page:restoredPage,profileArg})
    }
