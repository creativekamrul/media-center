...
const api: DesktopAPI = {
 ...
 onShortcut:fn=>{const cb=(_e:unknown,action:string)=>fn(action);ipcRenderer.on('app:shortcut',cb);return()=>ipcRenderer.removeListener('app:shortcut',cb)},
  ...
  localQueue: input => ipcRenderer.invoke('local:queue',input),
  ...
  appearanceRecovery:action=>ipcRenderer.invoke('theme:recovery',action),
  onSystemResume:listener=>{const cb=()=>listener();ipcRenderer.on('system:resume',cb);return()=>ipcRenderer.removeListener('system:resume',cb)},
