...
export class Store {
  ...
  resetData(scope:'cache'|'personal'|'all'){
    this.db.exec('BEGIN');try{
      ...
      else {this.db.exec("DELETE FROM listening; DELETE FROM kv WHERE key IN ('history','queue','savedQueues','notes','listenLater','smartPlaylists') OR key LIKE 'lyrics-binding:%' OR key LIKE 'lyrics-identity:%' OR key LIKE 'lyric-offset:%' OR key LIKE 'local-favorites:%' OR key LIKE 'local-playlists:%' OR key LIKE 'playlist-art:%' OR key LIKE 'personal-cover:%' OR key LIKE 'personal-cover-source:%' OR key = 'personal-library'")}
