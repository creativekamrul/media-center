...
import {z} from 'zod'
import {experienceSchema,type SearchGroup} from '../shared/experience'
...
export function registerExperience(handle:Handle,store:Store,player:Player,downloads:Downloads,local:LocalFiles,index:LocalLibrary,provider:(id:string)=>Navidrome|Audiobookshelf,window:()=>BrowserWindow,inbox:()=>Promise<InboxEpisode[]>,changed:()=>void,reset:()=>Promise<void>,undo?:UndoJournal){
 ...
 handle('experience:save',experienceSchema,i=>{const assigned=Object.values(i.shortcuts).filter(Boolean).map(s=>s.toLowerCase());if(new Set(assigned).size!==assigned.length)throw Error('Assign each shortcut only once.');store.set('experience',i);changed()})
