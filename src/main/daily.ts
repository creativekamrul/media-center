...
export const backupSchema = z.object({ format: z.literal('media-center-personal'), version: z.literal(1), preferences: preferenceSchema, dailySettings: dailySchema, servers: z.array(z.object({ id, provider: z.enum(['navidrome','audiobookshelf']), name, url: z.string().url().max(2048), username: z.string().max(256) }).strict()).max(100), folders: z.array(z.object({ id, name, path: z.string().max(4096) }).strict()).max(100), queues: z.array(savedQueueSchema).max(100), notes: z.array(noteSchema).max(10000), plans: z.array(planSchema).max(10000), rules: z.array(ruleSchema).max(100),extra:personalExtraSchema.optional() }).strict()
...
export function registerDaily(handle: Handle, store: Store, player: Player, downloads: Downloads, provider: (id: string) => Navidrome | Audiobookshelf, window: () => BrowserWindow,undo?:UndoJournal) {
  ...
  handle('home:get', z.boolean().optional(), async refresh => {
    ...
    const sampled=await Promise.allSettled(data.albums.slice(0,4).map(async album=>(await nav(album.serverId).collection('album',album.id)).tracks.slice(0,25).map(trackQueue)))
    const albumTracks:QueueItem[]=[]
    sampled.forEach(r=>{if(r.status==='fulfilled')albumTracks.push(...r.value);else data.warnings.push('Some album sampler tracks could not be loaded.')})
    data.mixes=homeMixes(favorites,history,albumTracks)
    return data
  })
  ...
  const saveFile = async (title:string,defaultPath:string,value:unknown) => {const body=JSON.stringify(value,null,2);if(Buffer.byteLength(body,'utf8')>25*1024*1024)throw Error('This backup exceeds the 25 MB import limit. Remove unused custom covers or saved selections before exporting.');const d=await dialog.showSaveDialog(window(),{title,defaultPath,filters:[{name:'Media Center JSON',extensions:['json']}]});if(d.canceled||!d.filePath)return false;await writeFile(d.filePath,body,'utf8');return true}
  ...
  handle('backup:export',z.undefined(),()=>saveFile('Export personal backup','media-center-backup.json',backupSchema.parse({format:'media-center-personal',version:1,preferences:store.preferences(),dailySettings:{...defaultDailySettings,...store.get('dailySettings') as object},servers:store.connections(),folders:store.get('localRoots')??[],queues:get('savedQueues'),notes:get('notes'),plans:store.laterList(),rules:get('smartPlaylists'),extra:exportExtra(store)})))
