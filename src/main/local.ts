...
export class LocalFiles {
  ...
  async metadata(rootId: string, fileId: string): Promise<LocalFile> {
    const path = await this.path(rootId, fileId), info = await stat(path), key = `metadata:${rootId}:${fileId}`
