...
export function registerFeatures(handle: Handle, store: Store, player: Player, local: LocalFiles, provider: (id: string) => Navidrome | Audiobookshelf, window: () => BrowserWindow, undo?:UndoJournal) {
  ...
  handle('playlist:save', z.object({ serverId: id, id: id.optional(), name: z.string().trim().min(1).max(200), comment: z.string().max(5000), public: z.boolean(), songIds: z.array(id).max(5000).optional() }).strict(), async i=>{const p=nav(i.serverId),before=i.id?await p.collection('playlist',i.id):undefined;const result=await p.savePlaylist(i);store.cacheClear();if(before?.playlist){const after=await p.collection('playlist',result.id);undo?.add('playlist edit',async()=>{const current=await p.collection('playlist',result.id);assertUnchanged({tracks:after.tracks.map(t=>t.id),name:after.title,comment:after.playlist?.comment,public:after.playlist?.public},{tracks:current.tracks.map(t=>t.id),name:current.title,comment:current.playlist?.comment,public:current.playlist?.public});await p.savePlaylist({id:result.id,name:before.title,comment:before.playlist!.comment,public:before.playlist!.public,songIds:before.tracks.map(t=>t.id)});store.cacheClear()})}return result})
