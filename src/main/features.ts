...
type Handle = <T extends z.ZodTypeAny>(channel: string, schema: T, action: (input: z.infer<T>) => unknown) => void
export function registerFeatures(handle: Handle, store: Store, player: Player, local: LocalFiles, provider: (id: string) => Navidrome | Audiobookshelf, window: () => BrowserWindow) {
  ...
  handle('music:favorite', z.object({ serverId: id, kind: z.enum(['song','album','artist']), id, favorite: z.boolean() }).strict(), i => nav(i.serverId).favorite(i.kind, i.id, i.favorite))
  handle('music:rate', z.object({ serverId: id, id, rating: z.number().int().min(0).max(5) }).strict(), i => nav(i.serverId).rate(i.id, i.rating))
  handle('playlist:save', z.object({ serverId: id, id: id.optional(), name: z.string().trim().min(1).max(200), comment: z.string().max(5000), public: z.boolean(), songIds: z.array(id).max(5000).optional() }).strict(), i => nav(i.serverId).savePlaylist(i))
  handle('playlist:delete', z.object({ serverId: id, id }).strict(), i => nav(i.serverId).deletePlaylist(i.id))
