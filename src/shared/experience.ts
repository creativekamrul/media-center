import {viewPinsSchema} from './view-pins'
import {z} from 'zod'
...
export const librarySelectionScopes=['music','musicPlaylists','local','localPlaylists','localFolders','audiobooks','podcasts','playlistSource'] as const
export type LibrarySelectionScope=typeof librarySelectionScopes[number]
export const librarySelectionsSchema=z.record(z.enum(librarySelectionScopes),z.string().max(4096)).default({})
export const navigationSchema=z.object({librarySelections:librarySelectionsSchema,viewPins:viewPinsSchema.default({}),collections:collectionsSchema.default(collectionDefaults),collectionOrder:collectionOrderSchema.default([...orderedCollectionKeys]),queueBelowHome:z.boolean().default(false),playlistsBelowHome:z.boolean().default(false),listeningCollapsed:z.boolean().default(false),sidebarCollapsed:z.boolean().default(false)}).strict()
export const navigationChangeSchema=z.discriminatedUnion('action',[z.object({action:z.literal('library-selection'),scope:z.enum(librarySelectionScopes),value:z.string().max(4096)}).strict(),z.object({action:z.literal('view-pins'),viewPins:viewPinsSchema.refine(v=>Object.keys(v).length===1&&Object.values(v)[0]!==undefined,'Save one tab bar at a time.')}).strict(),z.object({action:z.literal('collections'),collections:collectionsSchema,collectionOrder:collectionOrderSchema.optional(),queueBelowHome:z.boolean().optional(),playlistsBelowHome:z.boolean().optional()}).strict(),z.object({action:z.literal('listening'),collapsed:z.boolean()}).strict(),z.object({action:z.literal('sidebar'),collapsed:z.boolean()}).strict()])
