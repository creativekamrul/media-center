...
export function ImmersivePlayer({player,close}:{player:PlaybackState;close:()=>void}){
 ...
 return <section className={`immersive-player layout-${prefs.layout} ${queueOpen?'queue-is-open':''} backdrop-${prefs.background} lyric-animation-${prefs.animation} ${prefs.motion?'with-motion':'still'}`} aria-label="Immersive playing screen" style={lyricAppearanceStyle(prefs)}>
  ...
  <header className="immersive-header"><button className="secondary" autoFocus onClick={close}><ArrowLeft size={16}/>Classic view</button><span className="eyebrow">A LITTLE CLOSER TO THE MUSIC</span>
   <div className="immersive-header-actions">{prefs.layout!=='studio'&&<button className="secondary" aria-expanded={queueOpen} aria-controls="immersive-current-queue" onClick={()=>setQueueOpen(v=>!v)}>{queueOpen?'Hide queue':'Show queue'}</button>}<div ref={setToolbar} className="immersive-tools-slot"/>{canLyrics&&<button className="secondary" disabled={!ready} aria-pressed={prefs.showMusicLyrics} onClick={()=>{const next={...prefs,showMusicLyrics:!prefs.showMusicLyrics};setPrefs(next);void api.savePlayingScreenPreferences(next).catch(e=>{setPrefs(prefs);setError(message(e))})}}>{prefs.showMusicLyrics?'Show Now Playing':'Show lyrics'}</button>}
