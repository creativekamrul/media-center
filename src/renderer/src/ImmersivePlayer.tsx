...
export function ImmersivePlayer({player,close,onExitError}:{player:PlaybackState;close:()=>void;onExitError:(error:string)=>void}){
 ...
 return <section className={`immersive-player layout-${prefs.layout} ${queueOpen?'queue-is-open':''} backdrop-${prefs.background} lyric-animation-${prefs.animation} ${prefs.motion?'with-motion':'still'}`} aria-label="Immersive playing screen" style={lyricAppearanceStyle(prefs)}>
  <PlayingBackdrop background={prefs.background} item={item}/>
  <header className="immersive-header"><button className="secondary" autoFocus onClick={close}><ArrowLeft size={16}/>Classic view</button>
   <div className="immersive-header-actions">{prefs.layout!=='studio'&&<button className="secondary" aria-expanded={queueOpen} aria-controls="immersive-current-queue" onClick={()=>setQueueOpen(v=>!v)}>{queueOpen?'Hide queue':'Show queue'}</button>}<div ref={setToolbar} className="immersive-tools-slot"/>{canLyrics&&<button className="secondary" disabled={!ready} onClick={()=>{const next={...prefs,showMusicLyrics:!prefs.showMusicLyrics};setPrefs(next);void api.savePlayingScreenPreferences(next).catch(e=>{setPrefs(prefs);setError(message(e))})}}>{prefs.showMusicLyrics?'Show Now Playing':'Show lyrics'}</button>}
   <button className="secondary" aria-label="Toggle fullscreen" title="Fullscreen (F11); Escape to exit" onClick={()=>void api.fullscreen('toggle').catch(e=>setError(message(e)))}><Maximize size={17}/>Fullscreen</button><button className="secondary" disabled={!ready} onClick={()=>setOptions(true)}><Settings2 size={17}/>Appearance</button></div>
  </header>
