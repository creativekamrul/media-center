...
export function ImmersivePlayer({player,close}:{player:PlaybackState;close:()=>void}){
 const [toolbar,setToolbar]=useState<HTMLDivElement|null>(null),[prefs,setPrefs]=useState(defaultPlayingScreen),[ready,setReady]=useState(false),[options,setOptions]=useState(false),[error,setError]=useState(''),[page,setPage]=useState(0)
 ...
 return <section className={`immersive-player backdrop-${prefs.background} lyric-animation-${prefs.animation} ${prefs.motion?'with-motion':'still'}`} aria-label="Immersive playing screen" style={lyricAppearanceStyle(prefs)}>
  ...
  <header className="immersive-header"><button className="secondary" autoFocus onClick={close}><ArrowLeft size={16}/>Classic view</button><span className="eyebrow">A LITTLE CLOSER TO THE MUSIC</span>
   <div className="immersive-header-actions"><div ref={setToolbar} className="immersive-tools-slot"/>{canLyrics&&<button className="secondary" disabled={!ready} aria-pressed={prefs.showMusicLyrics} onClick={()=>{const next={...prefs,showMusicLyrics:!prefs.showMusicLyrics};setPrefs(next);void api.savePlayingScreenPreferences(next).catch(e=>{setPrefs(prefs);setError(message(e))})}}>{prefs.showMusicLyrics?'Show Now Playing':'Show lyrics'}</button>}
