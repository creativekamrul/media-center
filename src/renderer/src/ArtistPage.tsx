...
export function ArtistPage({request,back,error}:{request:ArtistRequest;back:()=>void;error:(s:string)=>void}){
 const [albumSort,setAlbumSort]=useState<TrackSort>('original')
 ...
 <><CollectionHeader><>{album?<Art item={{...album.album!,kind:'album'}}/>:selected.ref?<RefArt refItem={selected.ref}/>:<div className="art artist-local-art"><TrackArtwork item={items[0]}/></div>}</><div className="detail-heading"><p className="eyebrow">{album||localAlbum?'ALBUM':'ARTIST'} · {selected.source}</p><h1>{album?.title??localAlbum??selected.name}</h1><p>{album?album.subtitle:`${albums.length} ${albums.length===1?'album':'albums'} · ${items.length} ${items.length===1?'track':'tracks'}`}</p><ListenActions items={album?songQueue(sortedTrackIndexes(album.tracks,albumSort).map(i=>album.tracks[i]),album.title):localAlbum?items.filter(q=>q.context===localAlbum):items} disabled={busy} error={error} more={selected.ref&&!album&&!localAlbum?<PersonalActions refs={[selected.ref]}/>:undefined}/></div></CollectionHeader>
 {busy&&<p role="status">Loading artist collection…</p>}
 {album?<Tracks sortValue={albumSort} onSortChange={setAlbumSort} tracks={album.tracks} serverId={album.album!.serverId} context={album.title} error={error}/>:<>
