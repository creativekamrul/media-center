...
function LyricsContent({player,immersive=false,motion=true,wordLift=false,toolbar}:{player:PlaybackState;immersive?:boolean;motion?:boolean;wordLift?:boolean;toolbar?:HTMLElement|null}) {
 ...
 return <section className="lyrics-panel" aria-label="Lyrics">{immersive&&toolbar&&createPortal(<details className="immersive-lyric-menu"><summary>Lyrics tools</summary><div className="immersive-lyric-tools">{lyricTools}<button className="icon-button" aria-label="Refresh lyrics" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={18}/></button>{!!shown?.lines.length&&<details className="lyric-playback-options"><summary>Timing & follow</summary>{timingTools}</details>}</div></details>,toolbar)}<div className="section-title"><div><h2>{immersive?item?.title||'Lyrics':'Lyrics'}</h2>{!immersive&&<p className="muted">LRCLIB · {shown?.lines.length?'Synced to MPV':'Song lyrics'}</p>}</div>{!immersive&&<button className="icon-button" aria-label="Refresh lyrics" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={18}/></button>}</div>{!immersive&&lyricTools}{immersive?createPortal(dialogs,document.body):dialogs}{!immersive&&<p className="lyrics-caption">{item?.title} · {item?.subtitle}</p>}
 ...
 {!immersive&&<p className="lyrics-credit">Lyrics are looked up only while this panel is open. Song metadata is sent to LRCLIB; server credentials and audio are not.</p>}</section>
}
