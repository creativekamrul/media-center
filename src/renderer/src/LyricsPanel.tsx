...
function LyricsContent({player,immersive=false,motion=true,wordLift=false,toolbar}:{player:PlaybackState;immersive?:boolean;motion?:boolean;wordLift?:boolean;toolbar?:HTMLElement|null}) {
 ...
 const findButton=<button className="secondary" onClick={e=>{e.currentTarget.closest('details')?.removeAttribute('open');setSearchKey(key)}}><Search size={16}/>Find lyrics</button>
 const importTools=<><button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'file'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Import lyrics</button>{item?.target.kind==='local-file'&&<button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'embedded'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Read embedded lyrics</button>}<button className="secondary" disabled={loading} onClick={e=>{e.currentTarget.closest('details')?.removeAttribute('open');setEditKey(key)}}>Edit lyrics & timing</button>{shown?.saved&&<button className="secondary" onClick={()=>void api.clearLyrics({key}).then(()=>setRevision(n=>n+1)).catch(e=>setError(message(e)))}>Clear saved match</button>}</>
 const refreshButton=<button className="secondary" aria-label="Refresh lyrics" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={18}/>Refresh lyrics</button>
 const lyricTools=item&&['music-track','local-file'].includes(item.target.kind)&&<div className="lyrics-tools">{findButton}{immersive?importTools:<MoreOptions label="Lyric tools">{importTools}{refreshButton}{!!shown?.lines.length&&<div className="more-options-group"><button className="secondary" onClick={()=>setTimingOpen(true)}>Timing & follow</button></div>}<p className="lyrics-source-note">Embedded or server lyrics are checked before LRCLIB. Only song metadata is sent to LRCLIB.</p></MoreOptions>}</div>
