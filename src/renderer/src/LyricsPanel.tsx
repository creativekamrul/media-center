...
function LyricsContent({player,immersive=false,motion=true,wordLift=false,toolbar}:{player:PlaybackState;immersive?:boolean;motion?:boolean;wordLift?:boolean;toolbar?:HTMLElement|null}) {
 ...
 const [offset,setOffset]=useState(0),[timingOpen,setTimingOpen]=useState(false)
 ...
 useEffect(()=>{setSearchKey(undefined);setEditKey(undefined);setTimingOpen(false)},[key])
 ...
 const source=shown?.saved?'Saved lyrics':shown?.source==='embedded'?'Embedded lyrics':shown?.source==='navidrome'?'Navidrome':shown?.source==='lrclib'?'LRCLIB':'Song lyrics'
 const followControl=<label className="check-field"><input type="checkbox" checked={follow} onChange={e=>setFollow(e.target.checked)}/>Follow playback</label>
 ...
 const lyricTools=item&&['music-track','local-file'].includes(item.target.kind)&&<div className="lyrics-tools"><MoreOptions label="Lyrics tools">
  <div className="lyric-tool-status"><strong>{item.title}</strong><p>{source} · {shown?.lines.length?'Synced to MPV':'Song lyrics'}</p>{!!shown?.lines.length&&<>{followControl}<small>{offset?`Timing ${offset>0?'+':''}${offset}s`:'Click a line to seek'}</small></>}</div>
  <div className="more-options-group"><button className="secondary" onClick={()=>setSearchKey(key)}><Search size={16}/>Find lyrics</button><button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'file'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Import lyrics</button>{item.target.kind==='local-file'&&<button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'embedded'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Read embedded lyrics</button>}<button className="secondary" disabled={loading} onClick={()=>setEditKey(key)}>Edit lyrics & timing</button>{shown?.saved&&<button className="secondary" onClick={()=>void api.clearLyrics({key}).then(()=>setRevision(n=>n+1)).catch(e=>setError(message(e)))}>Clear saved match</button>}</div>
  <div className="more-options-group"><button className="secondary" aria-label="Refresh lyrics" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={18}/>Refresh lyrics</button>{!!shown?.lines.length&&<button className="secondary" onClick={()=>setTimingOpen(true)}>Timing & follow</button>}</div>
  <p className="lyrics-source-note">Embedded or server lyrics are checked before LRCLIB. Only song metadata is sent to LRCLIB.</p>
 </MoreOptions></div>
 return <section className="lyrics-panel" aria-label="Lyrics">{toolbar?createPortal(lyricTools,toolbar):lyricTools}{createPortal(dialogs,document.body)}{timingOpen&&createPortal(<Modal title="Lyric timing" close={()=>setTimingOpen(false)}>{timingTools}</Modal>,document.body)}
 ...
 {shown?.warning&&<p className="muted" role="status">{shown.warning}</p>}</section>
}
