...
function LyricsContent({player,immersive=false,motion=true,wordLift=false}:{player:PlaybackState;immersive?:boolean;motion?:boolean;wordLift?:boolean}) {
 ...
 return <section className="lyrics-panel" aria-label="Lyrics"><div className="section-title"><div><h2>Lyrics</h2><p className="muted">LRCLIB · {shown?.lines.length?'Synced to MPV':'Song lyrics'}</p></div><button className="icon-button" aria-label="Refresh lyrics" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={18}/></button></div>{item&&['music-track','local-file'].includes(item.target.kind)&&<div className="lyrics-tools"><button className="secondary" onClick={()=>setSearchKey(key)}><Search size={16}/>Find lyrics</button><button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'file'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Import lyrics</button>{item.target.kind==='local-file'&&<button className="secondary" disabled={loading} onClick={()=>void api.lyricImport({key,source:'embedded'}).then(ok=>{if(ok)setRevision(n=>n+1)}).catch(e=>setError(message(e)))}>Read embedded lyrics</button>}<button className="secondary" disabled={loading} onClick={()=>setEditKey(key)}>Edit lyrics & timing</button>{shown?.saved&&<><span className="muted">Saved match · #{shown.recordId}</span><button className="text-button" onClick={()=>void api.clearLyrics({key}).then(()=>setRevision(n=>n+1)).catch(e=>setError(message(e)))}>Clear saved match</button></>}</div>}{searchKey===key&&<LyricSearch key={key} songKey={key} title={item?.title??''} artist={item?.subtitle??''} close={()=>setSearchKey(undefined)} saved={()=>setRevision(n=>n+1)}/>}{editKey===key&&<LyricEditor songKey={key} player={player} result={result} close={()=>setEditKey(undefined)} saved={()=>setRevision(n=>n+1)}/>}<p className="lyrics-caption">{item?.title} · {item?.subtitle}</p>
