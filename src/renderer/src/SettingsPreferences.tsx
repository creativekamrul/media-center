...
export function ThemePreferences(){
  ...
  useEffect(()=>{const refresh=()=>{void api.appearanceRecovery('get').then(setRecovery).catch(e=>error(message(e)))};refresh();return api.onTheme(refresh)},[])
  async function recover(){try{setRecovery(await api.appearanceRecovery(recovery?'restore':'start'))}catch(e){error(message(e))}}
  ...
  return <section className="settings-panel theme-preferences"><label className="setting-toggle"><span><strong>Show page guidance</strong><small>Display tips and explanations throughout the app.</small></span><input type="checkbox" aria-label="Show page guidance" checked={prefs.appearance?.showHelp??true} onChange={e=>setPrefs(p=>({...p,appearance:{...(p.appearance??defaultAppearance),showHelp:e.target.checked}}))}/></label><StylePicker value={prefs} onChange={setPrefs}/><ThemePicker value={prefs.theme} onChange={theme=>setPrefs(p=>({...p,theme}))}/><AppearanceEditor value={prefs} onChange={setPrefs}/>
    ...
    <div className="appearance-recovery"><div><h3>Appearance recovery</h3><p className="muted">{recovery?'Your previous theme and CSS are saved. Restore them at any time.':'Use defaults if custom CSS or colors make the app difficult to use. Your saved appearance is kept.'}</p><small className="muted">Available anytime with Ctrl + Shift + F9 or from the tray menu.</small></div><button className="secondary" disabled={busy} onClick={()=>void recover()}>{recovery?'Restore saved appearance':'Use default appearance'}</button></div><div className="settings-actions"><button className="primary" disabled={busy} onClick={()=>void save('Theme saved')}>Save theme</button></div>{notice&&<span className="action-notice" role="status">{notice}</span>}
  </section>
}
