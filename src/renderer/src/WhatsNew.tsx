...
export function WhatsNew({navigate,error}:{navigate:(view:'home'|'settings'|'stats'|'now')=>void;error:(s:string)=>void}){
  ...
  return open?<Modal title={`What’s new in Media Center ${APP_VERSION}`} close={close}><div className="release-news"><p className="eyebrow">A HOME FOR EVERYTHING YOU LISTEN TO</p><p className="release-intro">A calmer library, clearer controls and a queue that stays beside your music.</p><div className="release-highlights">
    <article><Sparkles/><h3>Everything in its place</h3><p>Collection and episode actions now share a consistent layout. Open More options for downloads, progress and customization.</p><button className="text-button" onClick={()=>go('home')}>Explore your collection <ArrowRight size={15}/></button></article>
    <article><Search/><h3>Stay with your music</h3><p>Now Playing keeps artwork and song details stationary while the queue scrolls on its own.</p><button className="text-button" onClick={()=>go('now')}>Open Now Playing <ArrowRight size={15}/></button></article>
