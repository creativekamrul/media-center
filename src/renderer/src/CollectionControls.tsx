...
export function CollectionControls({error}:{error:(s:string)=>void}){
 const [prefs]=useExperience(),[draft,setDraft]=useState<Experience['navigation']['collections']>(collectionDefaults),[order,setOrder]=useState([...orderedCollectionKeys]),[queueBelowHome,setQueueBelowHome]=useState(false),[saving,setSaving]=useState(false),[notice,setNotice]=useState('')
 const saved=JSON.stringify({collections:prefs.navigation.collections,collectionOrder:prefs.navigation.collectionOrder,queueBelowHome:prefs.navigation.queueBelowHome})
 useEffect(()=>{const n=JSON.parse(saved);setDraft(n.collections);setOrder(n.collectionOrder);setQueueBelowHome(n.queueBelowHome)},[saved])
 const dirty=JSON.stringify(draft)!==JSON.stringify(prefs.navigation.collections)||JSON.stringify(order)!==JSON.stringify(prefs.navigation.collectionOrder)||queueBelowHome!==prefs.navigation.queueBelowHome
 async function save(){
  ...
  try{await api.saveNavigation({action:'collections',collections:parsed.data,collectionOrder:order,queueBelowHome});setDraft(parsed.data);setNotice('Collection navigation saved.')}catch(e){error(message(e))}finally{setSaving(false)}
 }
