...
export function ListenActions({ items, error, compact = false, disabled = false, hidePlay = false }: { items: QueueItem[]; error: (s: string) => void; compact?: boolean; disabled?: boolean; hidePlay?: boolean }) {
  ...
  return <><div className={`listen-actions ${compact ? 'compact' : ''}`}>
    ...
    {items.some(i=>['music-track','audiobook','podcast-episode'].includes(i.target.kind)) && <button title="Download original audio" aria-label="Download selection" className={compact?'icon-button':'secondary'} disabled={disabled||busy||items.length>1000} onClick={()=>void run(()=>api.downloadAdd(items),'Added to downloads')}><Download size={15}/>{!compact&&<span className="action-label">Download</span>}</button>}
