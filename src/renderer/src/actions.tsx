...
export function ListenActions({ items, error, compact = false, disabled = false, hidePlay = false, more, primaryActions, inMenu=false }: { items: QueueItem[]; error: (s: string) => void; compact?: boolean; disabled?: boolean; hidePlay?: boolean; more?:ReactNode;primaryActions?:ReactNode;inMenu?:boolean }) {
  ...
  return <><div ref={actionRoot} className={`listen-actions ${compact ? 'compact' : ''} ${more||items.some(i=>['music-track','audiobook','podcast-episode'].includes(i.target.kind))?'has-more':''}`} >
    ...
    {primaryActions}
    {inMenu&&items.some(i=>['music-track','audiobook','podcast-episode'].includes(i.target.kind))&&<button title="Download original audio" aria-label="Download selection" className="secondary" disabled={disabled||busy||items.length>1000} onClick={()=>void run(()=>api.downloadAdd(items),'Added to downloads')}><Download size={15}/><span className="action-label">Download</span></button>}
    {(more||!inMenu&&items.some(i=>['music-track','audiobook','podcast-episode'].includes(i.target.kind)))&&<MoreOptions>{items.some(i=>['music-track','audiobook','podcast-episode'].includes(i.target.kind))&&<button title="Download original audio" aria-label="Download selection" className="secondary" disabled={disabled||busy||items.length>1000} onClick={()=>void run(()=>api.downloadAdd(items),'Added to downloads')}><Download size={15}/><span className="action-label">Download</span></button>}{more&&<div className="more-options-group">{more}</div>}</MoreOptions>}
