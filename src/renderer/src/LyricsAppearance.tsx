 ...
 return <dialog ref={dialog} className="lyrics-appearance" aria-labelledby="lyrics-appearance-title" onCancel={e=>{e.preventDefault();cancel()}} onKeyDown={e=>{if(e.key==='Escape'){e.preventDefault();e.stopPropagation();cancel()}}} onClick={e=>{if(e.target===e.currentTarget)cancel()}}>
  ...
  <div className="lyrics-appearance-body">
   ...
   <fieldset disabled={busy} className="lyrics-option-grid"><legend className="sr-only">Lyrics customization</legend>
    ...
    <section><h3>Atmosphere & motion</h3><label>Immersive layout<select aria-label="Immersive layout" value={draft.layout} onChange={e=>update({layout:e.target.value as PlayingScreenPreferences['layout']})}><option value="studio">Studio · lyrics and queue</option><option value="minimal">Minimal · just the moment</option><option value="gallery">Gallery · album showcase</option></select></label><p className="muted guidance">Minimal keeps the cover and words with a queue you can open. Gallery puts a larger album cover beside your lyrics.</p><label>Background<select aria-label="Playing screen background" value={draft.background} onChange={e=>update({background:e.target.value as PlayingScreenPreferences['background']})}><option value="aurora">Aurora</option><option value="artwork">Blurred artwork</option><option value="midnight">Midnight glass</option><option value="sunset">Sunset</option><option value="stars">Starlight</option><option value="snow">Snowfall</option><option value="rain">Rain</option><option value="ocean">Deep ocean</option><option value="ember">Ember glow</option></select></label>
     {draft.background==='artwork'&&<label>Artwork blur · {draft.artworkBlur??48}px<input aria-label="Artwork blur" type="range" min={0} max={100} step={1} value={draft.artworkBlur??48} onChange={e=>update({artworkBlur:Number(e.target.value)})}/><small>Keep more detail or soften the background. The album cover stays sharp.</small></label>}
