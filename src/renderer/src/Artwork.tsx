...
export function TrackArtwork({item}:{item?:QueueItem}) {const ref=useRef<HTMLSpanElement>(null);useEffect(()=>{const row=ref.current?.closest('.local-library-track,.song-row,.track-row,.queue-row,.history-row,.unified-result,.personal-entry,.personal-shelf-card,.mix-preview-track,.local-file-row,.plan-card,.daily-card,.screen-queue-item,.home-resume')??ref.current;if(!row||!item)return;const show=(e:Event)=>{e.preventDefault();e.stopPropagation();window.dispatchEvent(new CustomEvent('track-menu',{detail:item}))};const key=(e:Event)=>{const k=e as KeyboardEvent;if(k.key==='ContextMenu'||(k.shiftKey&&k.key==='F10'))show(e)};row.addEventListener('contextmenu',show);row.addEventListener('keydown',key);return()=>{row.removeEventListener('contextmenu',show);row.removeEventListener('keydown',key)}},[JSON.stringify(item)]);return <span ref={ref} className="track-artwork" aria-hidden="true"><QueueArt key={JSON.stringify(item?.target)} item={item}/></span>}
