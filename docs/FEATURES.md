...
- **Cover discovery:** user-initiated MusicBrainz release search, with paced requests and a contact User-Agent; user-selected Cover Art Archive previews and matches. Network requests remain in main, redirects are restricted to provider hosts, and image bytes/dimensions are bounded. A saved match is retained locally. Album searches may not find every edition; local image import remains available.
- **Missing artwork:** branded peach/forest vinyl artwork is bundled for music covers that cannot be loaded. Selected public Cover Art Archive artwork can also be used in Discord. The same default image can be exported from Sharing settings for upload as the Discord application asset `media_center_default`; that optional fallback requires the user's asset upload because Discord cannot read bundled local files.
