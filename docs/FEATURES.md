...
- Local collection labels and a curated icon picker for the five collection navigation entries, with reset and explicit save in Appearance settings.
- Listening Space is a keyboard-accessible disclosure that persists its expanded/collapsed state.
- MPV configuration status is shown in Playback settings instead of occupying sidebar space.
- Navigation changes use a scoped validated IPC update, preserve unrelated settings drafts and are included in validated personal backups.
- The whole sidebar can collapse to a 76-pixel icon rail using the top-bar toggle. Collection icons, navigation tooltips, keyboard activation and selected-page highlights remain available; its saved state is independent of Listening Space.

## 1.1.0 personal library and listening improvements

- **Personal shelves:** locally saved, named shelves with descriptions, custom covers and manual item order. Albums, server playlists, artists, audiobook items, podcast shows and individual playable items can coexist. Duplicate identities are removed. Shelves resolve current server contents when played; shows resolve playable, unfinished episodes in their saved order. Up to 100 shelves, 500 references per shelf and 1,000 resolved playable items. Artist resolution is bounded to 50 albums.
- **Show preferences:** independent speed, intro/outro seconds and newest/oldest order, scoped by server and show ID. Explicit start positions and seeks are preserved. Intro skips do not rewind resumed episodes; outro skips are disabled when they would consume a very short episode. Applies to online and downloaded episodes; books retain their separate timeline behavior. An explicit per-episode speed override takes priority over the show default.
- **Offline preparation:** named plans from selections or shelves, original-file size where available, local-file/managed-download integrity checks, missing-file messages, download budget and download/retry actions. Sizes may be unavailable for music or partially reported by a server; there is no invented bitrate estimate. A plan is a saved selection, not an automatic background rule. Managed copies with missing files can be downloaded again. Local files must be restored by their owner; live radio is not downloadable.
- **People:** cross-library artist, author and narrator discovery, artist collection playback and book profiles, creator shortcuts from books/albums/track menus. Author/narrator searches inspect up to 10 book-library pages and show a limit warning. Uses collection metadata; no invented biographies or external author-photo service.
- **Metadata and cover customization:** validated device-local title, creator, album, author, narrator, genre, year and description overrides. Original audio/tags/server records are not written. Local library searches/grouping use the edited metadata. Server searches still use server metadata. Custom PNG/JPEG import is capped at 5 MB and 4096 pixels per side, decoded and normalized by the main process. Stored custom covers are capped at 18 MB total / 250 images.
- **Cover discovery:** user-initiated MusicBrainz release search, with paced requests and a contact User-Agent; user-selected Cover Art Archive previews and matches, with Apple iTunes album artwork fallback for failed or empty searches. Network requests remain in main, redirects are restricted to provider hosts, and image bytes/dimensions are bounded. A saved match is retained locally. Album searches may not find every edition; local image import remains available.
- **Missing artwork:** branded peach/forest vinyl artwork is bundled for music covers that cannot be loaded. Selected public Cover Art Archive or Apple iTunes artwork can also be used in Discord. Version 1.1.1 sends the already-published default image URL automatically when music artwork is missing; no per-user asset upload is required.
- **Saved list views:** named column visibility/order for music collection and indexed local track lists. Title/cover and essential playback controls remain available. Artist, album, duration, format, year, genre, rating and favorite columns are selectable; unsupported local rating controls are omitted. Server catalog sorting remains separate from presentation column order.
- **Private listening:** session-only toggle suppresses new device history, local stats, music scrobbles and Discord presence. Private elapsed time is not counted after disabling the mode. Book/episode resume checkpoints and server position sync remain available with zero listening-time delta; server-side streaming logs are outside the client's control. Explicit saves/downloads remain user-directed actions. Private mode starts off after relaunch.
- **Continue Listening:** Home, audiobook and podcast libraries share one responsive arrow slider and card component. Separate title, creator and progress rows prevent clipped creator text; horizontal scrollbar chrome is hidden while keyboard/trackpad scrolling remains available.
- Personal shelves, offline plans, metadata, show preferences, list views and custom covers are included in validated, source-remapped personal backups and the personal-data reset. Available in version 1.1.0.
...
## 1.2.0 — Custom mixes and collection navigation

- Custom mix recipes support up to eight condition groups (outer AND/OR; group AND/OR/NOT), with up to twelve conditions per group. Text: title, artist, album, genre and format. Numbers: year, duration in seconds, Navidrome rating and play count. Favorite and non-favorite rules work for Navidrome and local folders.
- Select up to ten music servers/folders, preview artwork and duration, then play, queue or save for later. Recipes save separately from the generated queue; preview again to regenerate. Sort by title/artist/album/year/duration/play count or shuffle, with a 1–1,000 track limit and optional limit per artist.
- Saved recipes appear on Home and in Library tools → Custom mixes, with editing, duplication and deletion. Personal backups remap source identities; personal-data reset removes recipes.
- Catalog scans are cached for two minutes; Refresh sources forces a rescan. Navidrome scans stop at 15,000 songs per selected server/library; local indexing retains its 50,000-file bound; a preview evaluates at most 100,000 songs overall. Warnings identify partial scans and source failures. Unknown numeric metadata does not satisfy numeric comparisons. BPM analysis, arbitrary-depth rule trees and scheduled regeneration are not implemented.
- Home's recently played albums loads 60 per server per page (or all returned if fewer) with incremental loading, identity deduplication and retryable source failures. It is no longer limited to eight. These are Navidrome recent albums, not fabricated local album history.
- Shared headers follow collection names/icons and active themes across Music, Local music, Audiobooks, Podcasts and Library tools. Underlined navigation replaces pill tabs. Dedicated spoken Continue listening views preserve explicit audiobook/episode targets.

### 1.2.0 interaction refinements

- Collection headers keep primary actions on one row; secondary actions are available from More options.
- Book/podcast Continue listening tabs use grids; Home retains its continuation slider.
- Optional Playlists shortcut below Home includes server and local playlists. Settings search fills the content width and People search controls align.
- Artist collection playback resolves its albums in main. Metadata display overrides preserve the strict playback request shape. Context menus support mouse right-click and Shift+F10 on track rows and collection cards; dialog errors remain visible inside dialogs.
