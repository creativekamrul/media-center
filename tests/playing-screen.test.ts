...
describe('Immersive appearance preferences',()=>{
 ...
 it('rejects unsafe CSS values, unknown fields and unbounded settings',()=>{
  for(const patch of [{wordColor:'url(https://example.com)'},{font:'untrusted'},{textColor:'#fff'},{glow:25},{inactiveOpacity:-1},{lineHeight:5},{backdropDim:1},{animation:'unknown'},{customCss:'x'},{layout:'unknown'}])expect(playingScreenSchema.safeParse({...defaultPlayingScreen,...patch}).success).toBe(false)
 })
 it('round trips a fully customized appearance',()=>{
  const prefs={...defaultPlayingScreen,showMusicLyrics:false,layout:'gallery' as const,font:'georgia' as const,wordColor:'#ff9900',animation:'focus' as const,alignment:'center' as const}
