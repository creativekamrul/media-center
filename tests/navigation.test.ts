...
import {experienceSchema,collectionDefaults,navigationChangeSchema} from '../src/shared/experience'
...
describe('collection navigation preferences',()=>{
 it('keeps older preferences and backups usable with default navigation',()=>{
  expect(experienceSchema.parse({}).navigation).toEqual({collections:collectionDefaults,listeningCollapsed:false,sidebarCollapsed:false})
 ...
 it('round-trips local names, icons and collapse state through personal backups',()=>{
  ...
  try{
   const prefs=experienceSchema.parse({navigation:{collections:{...collectionDefaults,podcasts:{name:'My shows',icon:'radio'}},listeningCollapsed:true,sidebarCollapsed:true}})
